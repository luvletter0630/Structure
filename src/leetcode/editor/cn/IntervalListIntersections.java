...
public class IntervalListIntersections {
    ...
    class Solution {
        public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
            ...
            while (m < firstLen && n < secondLen) {
                int lo = Math.max(firstList[m][0], secondList[m][0]);
                int hi = Math.min(firstList[n][0], secondList[n][0]);
                if (lo <= hi) {
                    reslist.add(new int[]{lo, hi});
                }

                if (firstList[m][1] < secondList[m][1]) {
                    m++;
                } else {
                    n++;
                }
